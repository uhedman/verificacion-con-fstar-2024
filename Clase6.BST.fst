...
: Lemma (requires is_bst t) (ensures is_bst (insert0 x t)) 
= match t with
  | L -> ()
  | N (l, z, r) ->
    if x = z then ()
...
let rec member_ok1 (x:int) (t:bst) 
: Lemma (requires member x t) (ensures in_tree x t) 
= match t with
  | L -> ()
  | N (l, y, r) ->
    if x < y then member_ok1 x l
    else if x > y then member_ok1 x r
    else ()

let rec in_treel (x y:int) (t:bst) 
: Lemma (requires in_tree x t /\ all_lt y t) (ensures x < y)
= match t with
  | L -> ()
  | N (l, z, r) -> 
    if in_tree x l then in_treel x y l
    else if in_tree x r then in_treel x y r
    else ()

let rec in_treer (x y:int) (t:bst) 
: Lemma (requires in_tree x t /\ all_gt y t) (ensures x > y)
= match t with
  | L -> ()
  | N (l, z, r) -> 
    if in_tree x l then in_treer x y l
    else if in_tree x r then in_treer x y r
    else ()

let rec member_ok2 (x:int) (t:bst) 
: Lemma (requires in_tree x t) (ensures member x t) 
= match t with
  | L -> ()
  | N (l, y, r) ->
    if in_tree x l then (
    ...
    else if in_tree x r then (
      member_ok2 x r;
      in_treer x y r
      )
    else ()

let member_ok (x:int) (t:bst) 
: Lemma (member x t == in_tree x t) 
= if member x t then member_ok1 x t
  else if in_tree x t then member_ok2 x t
  else ()
...
let rec all_lt_trans (x y : int) (t : bst0) 
: Lemma (requires all_lt x t /\ y >= x) (ensures all_lt y t) 
= match t with
  | L -> ()
  | N (l, _, r) -> (
    all_lt_trans x y l;
...
let rec all_gt_trans (x y : int) (t : bst0) 
: Lemma (requires all_gt x t /\ y <= x) (ensures all_gt y t) 
= match t with
  | L -> ()
  | N (l, _, r) -> (
    all_gt_trans x y l;
